...
values_a = xl["ProcessInstance.TargetObject.Name"].values
values_e = xl["OriginalActor.Name"].values
values_f = xl["Company Name"].values
...
for item_a in range(0, tamanho):
    comparador = [values_e[item_a], values_a[item_a], values_f[item_a]]
    lista = []
    for item_b in range(0, tamanho):
        if (
            comparador[0] == values_e[item_b]
            and comparador[1] == values_a[item_b]
            and comparador[2] != values_f[item_b]
